            ...
            <tbody>
                @forelse ($categories as $category)
                    <tr wire:key="cat-{{ $category->id }}">
                        ...
                        <td class="text-text-secondary">{{ $category->parent?->name ?? '—' }}</td>
                ...
                @empty
                    <tr><td colspan="4" class="py-8 text-center text-text-muted">No categories yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" wire:click.self="$set('showForm', false)">
            <form wire:submit="save" class="card w-full max-w-sm bg-surface-1 space-y-4">
                ...
                <div>
                    ...
                    <select wire:model="parent_id" class="field-input">
