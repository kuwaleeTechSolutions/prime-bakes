<div>
    ...
    <div class="mb-4 flex justify-end">
        <button wire:click="create" class="btn-primary">+ Add department</button>
    </div>
            ...
            <tbody>
                @forelse ($departments as $department)
                    <tr wire:key="dept-{{ $department->id }}">
                        ...
                        <td class="text-text-secondary">{{ $department->employees_count }}</td>
                        <td><span class="{{ $department->is_active ? 'pill-paid' : 'pill-pending' }}">{{ $department->is_active ? 'Active' : 'Inactive' }}</span></td>
                        <td class="text-right">
                            <button wire:click="edit({{ $department->id }})" class="text-text-accent text-xs hover:underline">Edit</button>
                            <button wire:click="delete({{ $department->id }})" class="ml-3 text-xs text-status-unpaid hover:underline">Deactivate</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="4" class="py-8 text-center text-text-muted">No departments yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    @if ($showForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" wire:click.self="$set('showForm', false)">
            <form wire:submit="save" class="card w-full max-w-sm bg-surface-1 space-y-4">
                <div class="font-medium">{{ $editingId ? 'Edit department' : 'Add department' }}</div>
                <div>
