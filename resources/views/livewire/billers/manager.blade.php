<div>
    ...
    <div class="mb-4 flex justify-end">
        <button wire:click="create" class="btn-primary">+ Add biller</button>
    </div>

    <div class="card overflow-x-auto">
        <table class="table-base">
            <thead><tr><th>Name</th><th>Company / branch</th><th>Phone</th><th>City</th><th>Status</th><th></th></tr></thead>
            <tbody>
                @forelse ($billers as $biller)
                    <tr wire:key="biller-{{ $biller->id }}">
                        <td class="font-medium">{{ $biller->name }}</td>
                        <td>{{ $biller->company_name }}</td>
                        <td>{{ $biller->phone_number }}</td>
                        <td>{{ $biller->city }}</td>
                        <td><span class="{{ $biller->is_active ? 'pill-paid' : 'pill-pending' }}">{{ $biller->is_active ? 'Active' : 'Inactive' }}</span></td>
                        <td class="text-right">
                            <button wire:click="edit({{ $biller->id }})" class="text-text-accent text-xs hover:underline">Edit</button>
                            <button wire:click="delete({{ $biller->id }})" class="ml-3 text-xs text-status-unpaid hover:underline">Deactivate</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="py-8 text-center text-text-muted">No billers yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    @if ($showForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" wire:click.self="$set('showForm', false)">
            <form wire:submit="save" class="card w-full max-w-lg bg-surface-1 space-y-4 max-h-[90vh] overflow-y-auto">
                <div class="font-medium">{{ $editingId ? 'Edit biller' : 'Add biller' }}</div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="field-label">Company / branch name</label>
