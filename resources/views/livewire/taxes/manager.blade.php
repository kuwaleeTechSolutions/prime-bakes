<div>
    ...
    <div class="mb-4 flex justify-end">
        <button wire:click="create" class="btn-primary">+ Add tax</button>
    </div>
            ...
            <tbody>
                @forelse ($taxes as $tax)
                    <tr wire:key="tax-{{ $tax->id }}">
                        ...
                        <td>{{ $tax->rate }}%</td>
                        <td><span class="{{ $tax->is_active ? 'pill-paid' : 'pill-pending' }}">{{ $tax->is_active ? 'Active' : 'Inactive' }}</span></td>
                        <td class="text-right">
                            <button wire:click="edit({{ $tax->id }})" class="text-text-accent text-xs hover:underline">Edit</button>
                            <button wire:click="delete({{ $tax->id }})" class="ml-3 text-xs text-status-unpaid hover:underline">Deactivate</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="4" class="py-8 text-center text-text-muted">No taxes yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    @if ($showForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" wire:click.self="$set('showForm', false)">
            <form wire:submit="save" class="card w-full max-w-sm bg-surface-1 space-y-4">
                <div class="font-medium">{{ $editingId ? 'Edit tax' : 'Add tax' }}</div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" wire:model="rate" class="field-input">
                    @error('rate') <p class="mt-1 text-xs text-status-unpaid">{{ $message }}</p> @enderror
                </div>
